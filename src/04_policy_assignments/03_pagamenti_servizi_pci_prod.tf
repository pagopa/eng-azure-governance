...
resource "azurerm_management_group_policy_assignment" "pagamenti_servizi_pci_prod_metrics_logs" {
  name                 = "${local.pagamenti_servizi_pci_prod_prefix}metricslogs"
  ...
  parameters = <<PARAMETERS
    {
        "logAnalyticsId": {
            "value": "${local.pagamenti_servizi_pci_prod_metrics_logs_pci_workspace_id}"
        }
    }
  PARAMETERS
}

resource "azurerm_role_assignment" "pagamenti_servizi_pci_prod_metrics_logs_monitoring_contributor" {
  scope                = data.azurerm_management_group.pagamenti_servizi_pci_prod.id
  role_definition_name = "PagoPA Audit Logs Contributor"
  principal_id         = azurerm_management_group_policy_assignment.pagamenti_servizi_pci_prod_metrics_logs.identity[0].principal_id
}

resource "azurerm_role_assignment" "pagamenti_servizi_pci_prod_metrics_logs_contributor_log_analytics" {
  scope                = local.pagamenti_servizi_pci_prod_metrics_logs_pci_workspace_id
  role_definition_name = "Log Analytics Contributor"
  principal_id         = azurerm_management_group_policy_assignment.pagamenti_servizi_pci_prod_metrics_logs.identity[0].principal_id
}
...
resource "azurerm_management_group_policy_assignment" "pagamenti_servizi_pci_prod_event_hub" {
...
resource "azurerm_management_group_policy_assignment" "pagamenti_servizi_pci_prod_kubernetes" {
  name                 = "${local.pagamenti_servizi_pci_prod_prefix}k8s"
