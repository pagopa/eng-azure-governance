...
resource "azurerm_policy_set_definition" "app_service_prod" {
  ...
  policy_definition_reference {
    ...
    parameter_values = jsonencode({
      listOfAllowedSku = {
        value = var.app_service_prod.listofallowedsku
      }
      listOfAllowedKind = {
        value = var.app_service_prod.listofallowedkind
      }
    })
  }
  ...
  policy_definition_reference {
    policy_definition_id = data.terraform_remote_state.policy_app_service.outputs.function_app_allowed_tls_id
    parameter_values     = jsonencode({})
  }

  # App Service apps should use the latest TLS version
  policy_definition_reference {
    policy_definition_id = data.terraform_remote_state.policy_app_service.outputs.app_service_allowed_tls_id
    parameter_values     = jsonencode({})
  }
