resource "azurerm_subscription_policy_assignment" "metrics_logs" {
  ...
  display_name         = "PagoPA PROD PCI Metric logs"
...
resource "azurerm_role_assignment" "metrics_logs_monitoring_contributor" {
...
resource "azurerm_role_assignment" "metrics_logs_contributor_log_analytics" {
