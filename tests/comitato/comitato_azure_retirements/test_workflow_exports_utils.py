...
def test_portal_link_from_identifier_handles_resource_ids_and_urls() -> None:
    assert (
        portal_link_from_identifier("https://example.com/doc")
...
def test_priority_label_respects_deadline_windows() -> None:
    ...
    assert (
        priority_label(retirement_date="2026-03-15", as_of_date=as_of_date) == "Critico"
    )
    assert (
        priority_label(retirement_date="2026-06-01", as_of_date=as_of_date)
        == "Prioritario"
    )
    assert (
        priority_label(retirement_date="2026-12-01", as_of_date=as_of_date)
        == "Da pianificare"
    )
    assert (
        priority_label(retirement_date="2028-01-01", as_of_date=as_of_date) == "Debito"
    )
