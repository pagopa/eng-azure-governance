locals {
  virtual_machine_scale_set_prod = {
...
resource "azurerm_policy_set_definition" "virtual_machine_scale_set_prod" {
  ...
  metadata = <<METADATA
    {
        "category": "${local.virtual_machine_scale_set_prod.metadata_category_name}",
        "version": "v1.0.0",
        "ASC": "true"
    }
METADATA
  ...
  policy_definition_reference {
    ...
    {
      "listOfAllowedSKU": {
        "value": ${jsonencode(var.virtual_machine_scale_set_prod.listofallowedskuname)}
      }
    }
    VALUE
  }
